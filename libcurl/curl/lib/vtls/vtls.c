                      ...
                      struct ssl_config_data *dest)
{
  ...
  if(source->CAfile) {
    dest->CAfile = _strdup(source->CAfile);
    if(!dest->CAfile)
      return FALSE;
  }
  else
  ...
  if(source->CApath) {
    dest->CApath = _strdup(source->CApath);
    if(!dest->CApath)
      return FALSE;
  }
  else
  ...
  if(source->cipher_list) {
    dest->cipher_list = _strdup(source->cipher_list);
    if(!dest->cipher_list)
      return FALSE;
  }
  else
  ...
  if(source->egdsocket) {
    dest->egdsocket = _strdup(source->egdsocket);
    if(!dest->egdsocket)
      return FALSE;
  }
  else
  ...
  if(source->random_file) {
    dest->random_file = _strdup(source->random_file);
    if(!dest->random_file)
      return FALSE;
  }
  else
                               ...
                               size_t idsize)
{
  ...
  clone_host = _strdup(conn->host.name);
  if(!clone_host)
